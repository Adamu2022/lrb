import { Controller, Get, Put, Post, Body, Query, UseGuards, Request, UsePipes, Logger } from '@nestjs/common';
...
export class NotificationSettingsController {
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Get notification settings for current user/organization' })
  @ApiResponse({ status: 200, description: 'Notification settings retrieved successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Forbidden' })
  @ApiQuery({ name: 'owner_type', required: false, description: 'Owner type (organization|user)' })
  ...
  async getNotificationSettings(
    ...
    if (ownerType === 'user' && ownerId !== req.user.id && req.user.role !== 'super_admin') {
    ...
    const settings = await this.notificationsService.getNotificationSettings(ownerType, ownerId);
    if (!settings) {
      return null;
    }
  ...
  async healthCheck(@Request() req): Promise<any> {
    ...
    try {
      ...
      await this.notificationsService.getNotificationSettings('user', req.user.id);
  ...
  @Roles('super_admin')
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Run network diagnostics for notification services' })
  ...
  async runDiagnostics(@Request() req): Promise<any> {
    ...
    try {
      const diagnostics = await this.networkDiagnosticsService.runComprehensiveDiagnostics();
      return {
        success: true,
        diagnostics,
      };
  ...
  async updateNotificationSettings(
    ...
    if (updateNotificationSettingsDto.owner_type === 'user' && 
        updateNotificationSettingsDto.owner_id !== req.user.id && 
        req.user.role !== 'super_admin') {
      throw new Error('Unauthorized to update these settings');
    }

    const settings = await this.notificationsService.updateNotificationSettings(updateNotificationSettingsDto, req.user.id);
    return this.notificationsService.getMaskedSettings(settings);
  }
  ...
  async testNotification(
    ...
    switch (testNotificationDto.provider) {
      case 'sms':
        if (!testNotificationDto.test_to) {
          return { success: false, message: 'Test phone number is required for SMS testing' };
        }
        break;
      case 'email':
        if (!testNotificationDto.test_email) {
          return { success: false, message: 'Test email address is required for email testing' };
        }
        break;
      case 'push':
        if (!testNotificationDto.test_device_token) {
          return { success: false, message: 'Test device token is required for push notification testing' };
        }
        break;
    }

    // Get user's settings for testing
    const settings = await this.notificationsService.getNotificationSettings('user', req.user.id);
    
    if (!settings) {
      return { success: false, message: 'No notification settings found for this user' };
    }
    ...
    if (!config) {
      return { success: false, message: `No configuration found for ${testNotificationDto.provider}` };
    }

    return this.notificationsService.testNotification(
      testNotificationDto.provider,
      config,
      testNotificationDto.test_to || testNotificationDto.test_email || testNotificationDto.test_device_token
    );
  }
  ...
  async sendNotification(
    ...
    if (!sendNotificationDto.scheduleId || sendNotificationDto.scheduleId <= 0) {
      throw new Error('Valid schedule ID is required');
    }
    if (!sendNotificationDto.channels || sendNotificationDto.channels.length === 0) {
