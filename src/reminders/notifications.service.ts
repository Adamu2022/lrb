...
export class NotificationsService {
  ...
  async updateNotificationSettings(
    ...
    if (settings.email_config?.password) {
      settings.email_config.encrypted_password = this.encryptionService.encrypt(
        settings.email_config.password,
      );
      delete settings.email_config.password;
    }

    if (settings.sms_config?.twilio_token) {
      settings.sms_config.encrypted_twilio_token = this.encryptionService.encrypt(
        settings.sms_config.twilio_token,
      );
      delete settings.sms_config.twilio_token;
    }
